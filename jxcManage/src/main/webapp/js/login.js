...
$(function() {
	...
	// 验证该用户是否已经登录
//	$.post("/login/isLogin.do", function(data) {
//		if (parseInt(data) == 1) {
//			checkBrowser()
//			/*
//			// 检测平台
...
//										$("#checkImg").attr("src",$("#basepath").val()+ "tool/VerificationCode.jsp?time="+ (new Date().getTime().toString()));
...
function action($name, $pwd, o) {
	$.post($("#basepath").val() + "/login/userLogin.do", o, function(d) {
//		data = parseInt(data);
		var data=d.statusCode;
		switch (data) {
		case -10:
			$("#login-btn").button('reset');
			alert("ip获取错误");
...
//			window.location.href = $("#basepath").val() + "page/console/index.jsp";
			window.location.href=$("#basepath").val()+"jump/topage.do?pagePath=page/console&pageName=index";
			// 跳转语句
...
function getLoginInfo(info, name, pwd, securityCode) {
	...
	return {
		...
		"passWord" : pwd,
		"loginDeviceOs" : os,
		"loginDeviceOsVersion" : osvr,
		"loginDevice" : device,
		"loginBrowser" : br,
		"loginBrowserCore" : brvr,
		"loginDeviceOther" : info,
		"loginStatus" : 0,
		"loginMode" : 2
	};
...
function checkBrowser() {
	...
	if (!(browser.versions.android || browser.versions.iPhone || browser.versions.iPad)) {
		...
		//alert("手机!")
		window.location.href = $("#basepath").val()+ "mobile/customer/search.jsp";
	}
}